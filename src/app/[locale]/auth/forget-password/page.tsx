...
const ForgetPass = () => {
  ...
  const handleOTPComplete = async(otp: string) => {
    ...
    try {
      const response = await fetch("https://test.jiovanilibya.org/api/user/verify-otp", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        toast.success(result.message)
          localStorage.setItem('token', JSON.stringify(result.data.access_token)); // Save token to localStorage
          localStorage.setItem('userInfo', JSON.stringify(result.data.user)); // Save User to localStorage
          updateUser(result.data.user)
          router.push('/auth/reset-password'); // Redirect to home on successful login
        setShowOTP(true);
      } else {
  ...
  return (
    <>
      ...
      <div>
        ...
        {!showOTP ? <SendOTP setShowOTP={setShowOTP} setCountry_code={setCountry_code} setPhone={setPhone}/> : <div className='flex flex-col items-center'>

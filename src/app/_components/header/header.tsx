...
const Header = () => {
  ...
  const initialDirection = isClient ? direction : 'ltr';
  const initialUser = isClient ? user : null;

  const handleSelect = (selected: { option: string; img?: string | StaticImageData }) => {
    toggleDirection();
    ...
    <>
      {isOpen && (
        <div
          ...
          className='fixed left-0 top-0 h-full w-full z-[9998] backdrop-blur-sm overflow-hidden'
        ></div>
      )}
      <div className={styles.top_header + ' py-1'}>
        <div className='flex justify-between max-w-[90%] mx-auto sm:max-w-xl md:max-w-2xl lg:max-w-4xl xl:max-w-6xl 2xl:max-w-7xl'>
          <ul className='flex justify-between gap-6 md:gap-2'>
            <li>
              <a href='#' className='flex items-center gap-0 md:gap-2'>
                <Image src={img1} alt='location icon' priority />
                <span className='hidden lg:inline'>No.28 Church Cres St, North Labone, Accra, Egypt</span>
              </a>
            </li>
            <li>
              <a href='#' className='flex items-center gap-0 md:gap-2'>
            ...
            <li>
              <a href='#' className='flex items-center gap-0 md:gap-2'>
                ...
                <span className='hidden lg:inline'>+2010 5654 1236</span>
              </a>
            </li>
          </ul>
      ...
      <nav className={styles.navbar + ' relative'}>
        <div className='flex justify-between items-center py-2 max-w-[90%] mx-auto sm:max-w-xl md:max-w-2xl lg:max-w-4xl xl:max-w-6xl 2xl:max-w-7xl'>
          ...
          <ul
            className={`fixed flex top-[0] h-[100vh] w-[15rem] bg-[#00431F] z-[9999] px-6 py-16 flex-col justify-start lg:static lg:h-auto lg:w-auto lg:bg-transparent lg:z-0 lg:px-0 lg:py-0 lg:flex-row gap-6 transition-all duration-500 ${
          ...
          >
            ...
            <li>
              <Link href='/our-projects' className='text-white lg:text-[#363636]'>
                Our Project
              </Link>
            </li>
            <li>
              <Link href='/news-articles' className='text-white lg:text-[#363636]'>
                News & Articles
              </Link>
            </li>
            <li>
            ...
            <li className='absolute lg:static bottom-4 left-0 w-full flex flex-col justify-center gap-4 p-8 lg:hidden'>
              <Link
                href='/auth/signup'
          ...
          <div className='flex items-center gap-4 md:gap-8'>
            <Dropdown
              ...
              placeholder={initialDirection === 'rtl' ? options[0] : options[1]}
              selected={initialDirection === 'rtl' ? options[0] : options[1]}
            />
            {initialUser ? (
              <Link href='/profile' className='text-[14px] font-[500] rounded-[4px] bg-white'>
                Hi, {initialUser.username}
              </Link>
            ) : (
              <div className='hidden gap-4 lg:flex lg:items-center'>
                <Link href='/auth/signup' className='text-[14px] font-[500] rounded-[4px] bg-white'>
                  Sign Up
                </Link>
                <Link
                  href='/auth/signin'
            ...
            <button className={styles.toggleMenu + ` flex lg:hidden`} onClick={() => setIsOpen(!isOpen)}>

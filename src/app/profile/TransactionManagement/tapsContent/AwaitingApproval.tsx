...
const RenderAwaitingApproval = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch('https://test.jiovanilibya.org/api/user/awaiting-approval', {
          headers: myHeaders,
        });
  ...
  return (
    <>
      <div className='flex flex-col lg:grid grid-cols-1 lg:grid-cols-3 gap-6'>
        {data && data.map(ele => <div key={ele.id} className='px-4 py-6 lg:px-6 lg:py-8  rounded-[20px] bg-[#fff] w-full'>
